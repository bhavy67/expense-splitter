        ...
        <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 p-5">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 flex items-center justify-center text-3xl shrink-0">
                {CATEGORY_EMOJIS[expense.category] ?? '📦'}
              </div>
              <div className="flex-1 min-w-0">
                <h1 className="text-xl font-bold text-gray-900 dark:text-zinc-100 truncate">
                  {expense.title}
                </h1>
            ...
            {payer && (
              <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-zinc-800">
                <span className="text-xs text-gray-500 dark:text-zinc-400">Paid by</span>
                <MemberDot member={payer} />
                <span className="text-sm font-medium text-gray-800 dark:text-zinc-200">{payer.name}</span>
              </div>
            )}
          </div>
            ...
            {expense.splitType === 'itemized' && expense.items?.length ? (
              <div>
                {expense.items.map((item) => (
                  <div key={item.id} className="px-4 py-3 border-b border-gray-50 dark:border-zinc-800/50 last:border-0">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-sm font-medium text-gray-800 dark:text-zinc-200">{item.name}</span>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {item.splits.map((s) => {
                        ...
                        if (!m) return null
                        return (
                          <div key={s.memberId} className="flex items-center gap-1 bg-gray-50 dark:bg-zinc-800 rounded-lg px-2 py-0.5">
                            <div className="w-3 h-3 rounded-full shrink-0" style={{ background: m.color }} />
                            <span className="text-xs text-gray-600 dark:text-zinc-400">{m.name}</span>
                            <span className="text-xs font-medium text-gray-800 dark:text-zinc-200 ml-1">
              ...
              <div>
                {expense.splits.map((split) => {
                  ...
                  return (
                    ...
                    >
                      <MemberDot member={m} />
                      <span className="flex-1 text-sm text-gray-800 dark:text-zinc-200">{m.name}</span>
                      {shares != null && (
          ...
          {expense.notes && (
            <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 p-4">
              <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wide mb-2">Notes</p>
              <p className="text-sm text-gray-700 dark:text-zinc-300 whitespace-pre-wrap">{expense.notes}</p>
            </div>
          )}
