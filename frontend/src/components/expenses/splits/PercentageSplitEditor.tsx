  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <span className={cn('text-xs font-semibold shrink-0', isValid ? 'text-emerald-600' : total > 100 ? 'text-red-500' : 'text-gray-600')}>
          {total.toFixed(0)}%
        </span>
      </div>
      ...
      {!isValid && (
        <div className="flex items-center justify-between">
          <p className={cn('text-xs', remaining > 0 ? 'text-amber-600' : 'text-red-500')}>
            {remaining > 0 ? `${remaining.toFixed(1)}% unassigned` : `${Math.abs(remaining).toFixed(1)}% over 100%`}
          </p>
          {remaining > 0 && (
            <button type="button" onClick={distributeRemaining} className="text-xs font-medium text-indigo-600 hover:underline">
              Distribute equally
            </button>
          )}
        </div>
        ...
        <div key={m.user.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-gray-200 bg-white">
          ...
          <div className="flex items-center gap-1.5">
            <input
              ...
              className="w-16 h-8 rounded-lg border border-gray-300 text-center text-sm font-semibold text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
